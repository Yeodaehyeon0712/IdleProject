...
public class AddressableSystem
{
    ...
    async UniTask LoadAssetAsync<T>(eAddressableState state, Dictionary<string, T> container)
    {
        ...
        {
            if (location.ResourceType != typeof(Texture2D)&&location.ResourceType != typeof(T))
            {
                Debug.LogError($"{location.PrimaryKey}Type missmatch");
                continue;
            }
            string loadKey = location.ResourceType == typeof(Texture2D)
            ? ConvertToSpriteKey(location.PrimaryKey):location.PrimaryKey;
    ...
    #endregion
    public string ConvertToSpriteKey(string input)
    {
        int startIndex = input.LastIndexOf('/') + 1;
        int endIndex = input.LastIndexOf('.');
        string name = input.Substring(startIndex, endIndex - startIndex);
        return  $"{input}[{name}].sprite";
    }
